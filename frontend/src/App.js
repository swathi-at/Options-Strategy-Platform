...
const translateFormToTrade = (strategy, form, currentSymbol) => {
    ...
    let tradeLegs = [];
    const symbol = currentSymbol; // This is now the "user friendly" key like 'NIFTY'
...
const findAtmStrike = (spot, symbolKey) => {
    // Find the config key (e.g., NIFTY)
    const configKey = Object.keys(SYMBOL_STRIKE_INCREMENT).find(key => key === symbolKey.toUpperCase());
    const increment = configKey ? SYMBOL_STRIKE_INCREMENT[configKey] : 50; // Default to 50
...
const autoFillPrimaryStrikes = (currentForm, currentStrategy, atmStrike, symbolKey) => {
    ...
    switch (currentStrategy) {
        ...
        case 'protective-put':
        case 'protective-call':
            newForm.stockPrice = Math.round(atmStrike / 10) * 10;
            newForm.strike = atmStrike;
            break;
        default:
            // Do nothing
...
function App() {
    ...
    const defaultFormState = {
        lots: 1,
        lotSize: SYMBOL_LOT_SIZES['NIFTY'], // Default to NIFTY
        targetPercent: 20,
        slPercent: -10
    };

    // `symbol` state now holds the Fyers API symbol ("NIFTY", "NSE:NIFTYAUTO-INDEX", etc.)
    ...
    const handleSymbolChange = (e) => {
        const newSymbol = e.target.value; // This is the Fyers symbol
        setSymbol(newSymbol);

        // Find the "config key" (e.g., NIFTY, BANKNIFTY) to look up lot size
        const configKey = Object.keys(SYMBOL_LOT_SIZES).find(key => key === newSymbol.toUpperCase());
        const newLotSize = configKey ? SYMBOL_LOT_SIZES[configKey] : 1; // Default to 1 if not a tradable F&O index
    ...
    const handleStrategyChange = (e) => {
        ...
        setForm(prevForm => ({
            ...defaultFormState,
            lotSize: prevForm.lotSize // Keep the lot size from previous symbol
        }));
        setData(null);
        setError(null);
        setAnalysis("");
    ...
    const handleChange = (e) => {
        ...
        // If in live mode and a strike changes, try to update premiums
    ...
    const handleReset = () => {
        ...
        setAnalysis("");
        setStrategy('');
        setSymbol('NIFTY'); // Reset to default NIFTY
        setIsLiveMode(false);
        setLiveData(null);
        setLiveDataError(null);
        
        // Re-select strategy and symbol to trigger form fields
        setTimeout(() => {
            setSymbol(currentSymbol);
            setStrategy(currentStrategy);
            // Re-apply lot size for the symbol
    ...
    const handleSubmit = async () => {
        ...
        setIsLoading(true);
        try {
            const payload = { strategy, ...form, symbol: symbol }; // Pass symbol
            // Ensure we use a valid lot size
    ...
    const handleAnalysis = async () => {
        if (!data || !API_KEY || API_KEY.includes("YOUR_API_KEY")) {
        ...
        try {
            ...
            setAnalysis(text || "No analysis could be generated.");
        } catch (error) {
    ...
    const handleSimulateTrade = async () => {
        ...
        // Find the "user friendly" key (e.g., NIFTY, BANKNIFTY)
        const symbolKey = liveData?.symbol || // From live data
                          Object.keys(SYMBOL_LOT_SIZES).find(key => key === symbol.toUpperCase()) || // From config
                          symbol; // Fallback
        ...
        try {
            const res = await axios.post(`${BACKEND_URL}/api/paper-trade`, tradePayload);
    ...
    const toggleLiveMode = async () => {
        if (isLiveMode) {
            ...
            setLiveData(null);
            setLiveDataError(null);
            setForm(prevForm => ({ ...prevForm, ...defaultFormState, lotSize: prevForm.lotSize }));
        } else {
            // Turning ON
            setLiveDataLoading(true);
            setLiveDataError(null);
            try {
                // `symbol` state already holds "NIFTY" or "NSE:NIFTYAUTO-INDEX"
                const res = await axios.get(`${BACKEND_URL}/api/live-data/${symbol}`);
                const liveApiData = res.data; // e.g., { symbol: "NIFTY", spot: 12345, options: [...] }
                setLiveData(liveApiData);
                setIsLiveMode(true);

                // --- Automation ---
                const symbolKey = liveApiData.symbol; // This is the "user friendly" key
                ...
                // Only fill premiums if options data exists
                ...
                const errorMsg = err.response?.data?.details || err.response?.data?.error || 'Failed to fetch live data.';
                if (err.response?.status === 401) {
                    setLiveDataError('Live data failed. Please re-login to Fyers via the backend.');
                } else {
                    setLiveDataError(`${errorMsg} (Symbol: ${symbol})`);
                }
                setIsLiveMode(false);
            } finally {
    ...
    // --- RENDER ---
    // Find if the currently selected symbol is a "tradable" one (has lot size config)
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto font-sans bg-gray-50 dark:bg-gray-900 min-h-screen">
            ...
            {/* --- [NEW] Algo Dashboard --- */}
            {/* This component will connect to ws://localhost:8080 and show live bot status */}
            <AlgoDashboard />
            ...
            <div className="mt-8 p-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-md mb-8">
                ...
                <div className="flex justify-between items-center mb-4 border-b dark:border-gray-600 pb-2">
                    ...
                    <div className="flex items-center space-x-2">
                        <label htmlFor="liveModeToggle" className={`text-sm font-medium ${liveDataLoading ? 'text-yellow-600' : (isLiveMode ? 'text-green-600' : 'text-gray-700 dark:text-gray-300')}`}>
                        ...
                        <button
                            ...
                            className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors disabled:opacity-50 ${isLiveMode ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                        >
                            <span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${isLiveMode ? 'translate-x-6' : 'translate-x-1'}`} />
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 items-end">
                    ...
                    <div className="col-span-2 md:col-span-1">
                        ...
                        >
                            {/* Dynamically create <optgroup> and <option> tags */}
                            {Object.entries(SYMBOL_LIST).map(([groupName, symbols]) => (
                    ...
                    {strategy && strategyConfigs[strategy]?.fields.map(field => {
                        // Conditionally hide lotSize if the symbol isn't in the tradable config
                        if (field === 'lotSize' && !currentConfigKey) {
                            return null;
                        }
                        // Conditionally hide all option fields if not a tradable index
                        if (!currentConfigKey && (field.startsWith('premium') || field.startsWith('strike') || field === 'stockPrice')) {
                             return null;
                        }
                        ...
                        return (
                            <div key={field} className="col-span-1">
                                ...
                                <input
                                    ...
                                    disabled={(isLiveMode && field.startsWith('premium')) || (field === 'lotSize' && !!currentConfigKey)}
                                    ...
                                    ${(isLiveMode && field.startsWith('premium')) || (field === 'lotSize' && !!currentConfigKey) ? 'bg-gray-100 dark:bg-gray-800' : ''}
                                    `}
                                    placeholder="0"
                                />
                            </div>
                ...
                <div className="mt-8 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl animate-fade-in">
                    ...
                    <div className="flex justify-between items-center mb-6 border-b dark:border-gray-600 pb-2">
                        ...
                        <div className="flex space-x-2">
                            {/* Only show Simulate button if it's a tradable index */}
                            {currentConfigKey && (
                                <button
                    ...
                    {/* Only show the chart if we have payoff data */}
                    {data.payoffCurve && data.payoffCurve.length > 0 ? (
                        ...
                        <div className="text-center text-gray-500 dark:text-gray-400 p-8">
                            Payoff chart is not available for this instrument (e.g., non-tradable indices) or for this strategy.
                        </div>
                    )}
                </div>
            )}

            {/* This panel will now render correctly */}
