...
function getSensibullConfig(symbol) {
    ...
    // 🔴 FIX: Check for BOTH 'BANKNIFTY' and 'NIFTYBANK'
    if (s.includes('BANKNIFTY') || s.includes('NIFTYBANK')) {
        return { width: 500, interval: 100 }; 
    } 
    else if (s.includes('FINNIFTY')) {
        return { width: 100, interval: 50 };
    } 
    else if (s.includes('MIDCP') || s.includes('MIDCPNIFTY')) {
        return { width: 75, interval: 25 };
    } 
    else if (s.includes('SENSEX')) {
        return { width: 300, interval: 100 };
    }
    
    // DEFAULT (NIFTY & OTHERS)
